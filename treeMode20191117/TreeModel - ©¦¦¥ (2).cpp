...
{
	bool result = insertItem(index.row(), index, value);
	if (result)
	{
...
bool TreeModel::insertItem(int intRow, const QModelIndex &index, const QVariant &value)
{
	//beginInsertRows(position+index.row() - 1);
	//endInsertRows();
	//return true;
	Entity  *entity = new(std::nothrow)  Entity;
	if (!index.isValid())
	{
		m_pEntity = m_pEntity->parentEntity;
	}
	else
	{
		m_pEntity = static_cast <Entity*>(index.internalPointer());
	}
	entity->parentEntity = m_pEntity;//insertChile()   setParent()
	m_pEntity->setData(value);
	return true;
}
	...
	{	
		...
		for (int i = 0; i < indexes.size();++i)
		{
			...
			if (index.isValid())
			{
				...
				//QVariant v = QVariant::fromValue <entityViewItemList>(parentData->children);
				//encoded.append(v.toChar());
				encoded.append(parentData->name);
			}
		}
